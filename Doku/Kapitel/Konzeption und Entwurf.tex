...
\newcounter{req}
\newcounter{reqTotalCompleted}
\newcounter{reqFunCompleted}
\newcounter{reqOptCompleted}
\newcounter{reqFunAmount}
\newcounter{reqOptAmount}
\newcommand*{\reqautorefname}{Anforderung}

\edef\fullreqs{}

\NewDocumentEnvironment{freq}{m o d()}{%
	\def\realreq{\number\numexpr\value{req}+1\relax}
	\subsubsection*{\IfNoValueTF{#2}{[FA--\ifnum\realreq<10 0\fi\realreq] #1}{(FA--\ifnum\realreq<10 0\fi\realreq) Optional -- #1}}%
	\refstepcounter{req}%
	\global\eappto\fullreqs{\thereq,freq,#1,#2,#3;}
	\label{freq\thereq}%
	\IfNoValueTF{#2}{\refstepcounter{reqFunAmount}}{\refstepcounter{reqOptAmount}}%
	\IfNoValueTF{#3}{}{\refstepcounter{reqTotalCompleted}\IfNoValueTF{#2}{\refstepcounter{reqFunCompleted}}{\refstepcounter{reqOptCompleted}}}
}{%
}
\NewDocumentEnvironment{nreq}{m o d()}{%
	\def\realreq{\number\numexpr\value{req}+1\relax}
	\subsubsection*{\IfNoValueTF{#2}{[NFA--\ifnum\realreq<10 0\fi\realreq] #1}{(FA--\ifnum\realreq<10 0\fi\realreq) Optional -- #1}}%
	\refstepcounter{req}%
	\global\eappto\fullreqs{\thereq,nreq,#1,#2,#3;}
	\label{nreq\thereq}
	\IfNoValueTF{#2}{\refstepcounter{reqFunAmount}}{\refstepcounter{reqOptAmount}}%
	\IfNoValueTF{#3}{}{\refstepcounter{reqTotalCompleted}\IfNoValueTF{#2}{\refstepcounter{reqFunCompleted}}{\refstepcounter{reqOptCompleted}}}
}{%
...
\end{freq}
\begin{freq}{Dynamische Laufzeitübersetzung von JavaFX-Komponenten}(c)
...
\end{freq}
\begin{freq}{Annotationsbasierte Controllerdefinition}(c)
...
\end{freq}
\begin{freq}{Unterstützung von mehreren Lebenszyklusbehandlungsmethoden}(c)
...
\end{freq}
\begin{freq}{Controller Preloading}(c)
...
\end{freq}
\begin{freq}{Serialisierung von JavaFX Komponenten}[o]
...
Auf der einen Seite ist das Nutzen von externen Bibliotheken im Vergleich zum manuellen Erstellen der gleichen Funktionen ein effektiver Weg um Zeit zu sparen und Bugs zu vermeiden. Auf der anderen Seite kann das Nutzen von vielen externen Bibliotheken, aufgrund der verschiedenen Softwarelizenzen für quelloffene Projekte, zu Problemen führen, da nicht alle Lizenzen eine Kompatibilität untereinander aufweisen. Für das Projekt werden nur Bibliotheken verwendet, welche lizenztechnisch kompatibel sind. Alle verwendeten, externen Bibliotheken sind mit einer Kurzbeschreibung der Funktion und der jeweiligen Lizenz in \autoref{tab:external_libraries} dargestellt.
\newcolumntype{C}[1]{%
