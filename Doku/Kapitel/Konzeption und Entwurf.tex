...
\newcounter{freq}
\newcommand*{\freqautorefname}{Anforderung}

\newenvironment{freq}[2][]{%
	\def\realfreq{\number\numexpr\value{freq}+1\relax}
	\subsubsection*{\ifblank{#1}{[FA--\ifnum\realfreq<10 0\fi\realfreq] #2}{(FA--\ifnum\realfreq<10 0\fi\realfreq) Optional -- #2}}%
	\refstepcounter{freq}%
	\label{freq\thefreq}
}{%
}

%\newcommand*{\freq}[2][]{\refstepcounter{fcounter}\subsubsection*{[FA--\thefcounter %#1]}\label{freq\thefcounter}}
...
\add{Maybe add functionality}
%Textbasierte Properties und Bindings sind hiervon ausgeschlossen. Der Annotation muss mindestens der Übersetzungsschlüssel übergeben werden und dieser muss in einem von \texttt{SimpliFX} zugänglichen \texttt{ResourceBundle} enthalten sein.
...
\add{Complete when more information of injection process}
